...
Public Partial Class RemoteMailBox
	...
	Private Sub bLogin_Click(sender As Object, e As EventArgs)
		...
		Try
			'Login to the server
			Dim response As CompletionResponse = _client.Login()
			...
			Dim folders As Mailbox = _client.GetMailboxTree()

			'Get inbox folder
			Dim inbox As Mailbox = Mailbox.Find(folders, "INBOX")

			'Get messages for the server
			Dim messages As MessageCollection = _client.GetAllMessageHeaders(inbox)
	...
	Private Sub FillMessageSList(messages As MessageCollection, inbox As Mailbox)
		'Clear messages list
		lvMessageBrowser.Items.Clear()

		'Adding messages to list
		For Each messageHeader As ImapMessage In messages
			'Get message from the server
			Dim uid As UInteger = Convert.ToUInt32(messageHeader.UID)
			Dim message As ImapMessage = _client.GetFullMessage(uid, inbox)
	...
	Private Sub lvMessageBrowser_DoubleClick(sender As Object, e As EventArgs)
		If lvMessageBrowser.SelectedItems.Count > 0 Then
			...
			Dim viewer As New MessageViewer(DirectCast(lvMessageBrowser.SelectedItems(0).Tag, ImapMessage))
			viewer.ShowDialog()
		End If
	End Sub
End Class
